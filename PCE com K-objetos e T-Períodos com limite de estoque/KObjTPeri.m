...
% períodos e que considera o estoque de itens e limite de estoque
...
% limites de estoque total
limt = 500;
% limites de estoque por tipo de item
iotr = 50;

% limit. de tempo do cplex
mik = 0.5;
...
% Produção nos períodos  OOOOKKKKK
produP = [];
for ij=1:nd
        inu = (1:npa) + (ij-1)*npa;
...
% produção de itens por objeto nos períodos OOOOKKKKKKK
produO = [];
for ij=1:(rh-1)
    a = IroK(1,ij);
    b = (IroK(1,ij+1)-1);
...
nam = num2str(koi);
csvwrite(['CSV/',nam,'WLUsados.csv'],Protok)
